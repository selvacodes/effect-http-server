...
const allRoutesCombined = Api.make().pipe(Api.addGroup(userApiSpecAsGroup))
	.pipe(Api.addGroup(randomApiGroup))
	.pipe(Api.addGroup(AliveApiGroup))

const routesWithHandler = ExampleServer.make(allRoutesCombined).pipe(RouterBuilder.build)
